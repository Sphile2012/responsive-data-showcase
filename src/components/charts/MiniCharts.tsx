...
function GridLines({
  ticks, padL, padT, w, h,
}: { ticks: number[]; padL: number; padT: number; w: number; h: number }) {
  return (
    <>
      {ticks.map((t, i) => (
        <line
          key={i}
          x1={padL} y1={padT + t * h}
          x2={padL + w} y2={padT + t * h}
          stroke={GRID} strokeDasharray="4 4"
        />
      ))}
    </>
  );
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
      ...
      {[110,150,190,230,270].map((v, i) => (
        <text key={i} x={PL - 6} y={yS(v) + 4} textAnchor="end" fontSize={9} fill={AXIS}>{v}</text>
      ))}
      {/* Grid */}
      ...
      {/* Target dashed line */}
      <path d={linePath(targetPts)} fill="none" stroke={PINK} strokeWidth={2}
        strokeDasharray="6 4" strokeOpacity={0.6} />
      {/* Sales line with glow */}
      <path d={linePath(salePts)} fill="none" stroke="url(#line-cv)" strokeWidth={3}
        filter="url(#gc)" />
      {/* Dots */}
      {salePts.map((p, i) => <Dot key={i} x={p.x} y={p.y} color={CYAN} />)}
      {/* X-axis labels */}
      {months.map((m, i) => (
        <text key={i} x={xS(i)} y={H - 4} textAnchor="middle" fontSize={9} fill={AXIS}>{m}</text>
      ))}
      {/* Legend */}
      <g>
        <rect x={PL}   y={PT - 2} width={20} height={3} rx={1.5} fill={CYAN} />
        <text x={PL + 24} y={PT + 2} fontSize={9} fill={AXIS}>Sales</text>
        <line x1={PL + 70} y1={PT} x2={PL + 90} y2={PT} stroke={PINK} strokeWidth={2}
          strokeDasharray="5 3" />
        <text x={PL + 94} y={PT + 2} fontSize={9} fill={AXIS}>Target</text>
      </g>
    </svg>
  );
}
...
export function RegionBarChart({ height = 240 }: { height?: number }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
        ...
        return (
          <g key={i} filter={`url(#g${["c","v","p","g","a"][i] ?? "c"})`}>
            ...
            <rect x={bx} y={by} width={barW} height={bh}
              rx={radius} fill={colors[i]} fillOpacity={0.15} />
            <rect x={bx} y={by} width={barW} height={bh}
              rx={radius} fill={`url(#bar-${i})`} />
            {/* Value label */}
            <text x={cx} y={by - 5} textAnchor="middle" fontSize={9} fill={colors[i]} fontWeight={700}>
              {v}
            </text>
          </g>
        );
      ...
      {/* X-axis labels */}
      {labels.map((l, i) => (
        <text key={i} x={PL + i * groupW + groupW / 2} y={H - 4}
          textAnchor="middle" fontSize={9} fill={AXIS}>{l}</text>
      ))}
    </svg>
  );
}
...
export function QualityPieChart({ height = 240 }: { height?: number }) {
  ...
  const OR = H * 0.38, IR = H * 0.21;

  const data   = [{ n: "Complete", v: 64 }, { n: "Missing", v: 18 }, { n: "Duplicate", v: 11 }, { n: "Invalid", v: 7 }];
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
      ...
      <text x={CX} y={CY - 8} textAnchor="middle" fontSize={22} fontWeight={700}
        fill="white" fontFamily="monospace">64%</text>
      <text x={CX} y={CY + 12} textAnchor="middle" fontSize={10} fill={AXIS}>complete</text>

      {/* Legend on the right */}
      {data.map((d, i) => {
        const lx = W * 0.62, ly = CY - (data.length - 1) * 22 + i * 44;
        return (
          <g key={i}>
            <rect x={lx} y={ly - 6} width={10} height={10} rx={5} fill={colors[i]}
              style={{ filter: `drop-shadow(0 0 4px ${colors[i]})` }} />
            <text x={lx + 16} y={ly + 3} fontSize={11} fill="#94a3b8"
              fontFamily="Inter, sans-serif">{d.n}</text>
            <text x={W - 14} y={ly + 3} textAnchor="end" fontSize={12} fontWeight={700}
              fill={colors[i]} fontFamily="monospace">{d.v}%</text>
          </g>
        );
      })}
    </svg>
...
export function HivPrevalenceChart({ height = 220 }: { height?: number }) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ overflow: "visible" }}>
      ...
      {/* Dots */}
      {prevPts.map((p, i) => <Dot key={`p${i}`} x={p.x} y={p.y} color={CYAN}   />)}
      {artPts.map((p, i)  => <Dot key={`a${i}`} x={p.x} y={p.y} color={VIOLET} />)}
      {/* X labels */}
      {years.map((y, i) => (
        <text key={i} x={xS(i)} y={H - PB + 16} textAnchor="middle" fontSize={9} fill={AXIS}>{y}</text>
      ))}
      {/* Last values */}
      <text x={xS(5) + 8} y={prevY(14.0) + 4}  fontSize={10} fill={CYAN}   fontWeight={700}>14.0%</text>
      <text x={xS(5) + 8} y={artY(82)    + 4}   fontSize={10} fill={VIOLET} fontWeight={700}>82%</text>
      {/* Legend */}
      <g>
        <rect x={PL} y={PT + ch + 22} width={22} height={3} rx={1.5} fill={CYAN} />
        <text x={PL + 26} y={PT + ch + 26} fontSize={9} fill={AXIS}>Prevalence %</text>
        <rect x={PL + 110} y={PT + ch + 22} width={22} height={3} rx={1.5} fill={VIOLET} />
        <text x={PL + 136} y={PT + ch + 26} fontSize={9} fill={AXIS}>ART Coverage %</text>
      </g>
    </svg>
  );
}
